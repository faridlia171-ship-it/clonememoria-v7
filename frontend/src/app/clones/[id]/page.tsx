...
export default function CloneDetailPage() {
  ...
  // Protection : vérification d’existence
  ...
  useEffect(() => {
    if (!cloneId) {
      logger.error('Invalid cloneId received in page params');
  ...
  const loadClone = async () => {
    try {
      ...
      // Protection données null
      if (!data || typeof data !== 'object') {
        throw new Error('Invalid clone data structure');
      }

      setClone(data);
      logger.info('Clone loaded', { cloneId, cloneName: data?.name ?? 'unknown' });
  ...
  // État de chargement ou clone non disponible
  if (loading || !clone) {
    return (
      <AppLayout>
  ...
  const description = clone.description ?? 'No description provided';
          ...
          <Link href={`/clones/${cloneId}/chat`}>
            <div className="card hover:shadow-soft-lg transition-shadow duration-200 cursor-pointer h-full">
              <div className="flex items-center space-x-4 mb-4">
                ...
                <h2 className="text-2xl font-display text-gray-900">Chat</h2>
              </div>
              <p className="text-gray-600">Start a conversation with {name}</p>
            </div>
          </Link>
          ...
          <Link href={`/clones/${cloneId}/memories`}>
            <div className="card hover:shadow-soft-lg transition-shadow duration-200 cursor-pointer h-full">
              <div className="flex items-center space-x-4 mb-4">
                ...
                <h2 className="text-2xl font-display text-gray-900">Memories</h2>
              </div>
              <p className="text-gray-600">
                View and add memories for {name}
              </p>
            </div>
          </Link>
